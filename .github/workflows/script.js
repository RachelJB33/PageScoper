...
function getStatusBadgeClass(status) {
  ...
  if (status === "Planning") {
    return "bg-blue-500/30 text-white border-none";
  }

  if (status === "Design") {
    return "bg-purple-500/30 text-white border-none";
  }

  if (status === "Build") {
    return "bg-pink-500/30 text-white border-none";
  }

  if (status === "Launched") {
    return "bg-green-500/30 text-white border-none";
  }

  return "bg-slate-500/30 text-white border-none";
}
...
if (projects.length === 0) {
  ...
  projectList.innerHTML = `
    <div class="card bg-slate-800 border border-slate-700 rounded-2xl shadow-md sm:col-span-2">
  ...
  filteredProjects.forEach(function(project, index) {
    ...
    projectCard.className = "card bg-slate-800 border border-slate-500 rounded-2xl shadow-md hover:shadow-2xl hover:-translate-y-0.5 transition-all duration-200";
      ...
      ${project.isDeleted === true
        ? `
          <div class="flex flex-col gap-2 sm:flex-row">
            <button class="btn btn-sm !shadow-none bg-indigo-600/90 hover:bg-indigo-500 text-white border border-indigo-500/40 transition-all duration-150 restore-btn">Restore</button>
